...
open class MongoGenericService(cache: ICache) : CacheService(cache) {
    ...
    fun <T: Any> findAll(col: CoroutineCollection<T>, filter: Bson) = runBlocking{
        col.find(filter).toList()
    }

    fun <T: Any> findPage(col: CoroutineCollection<T>, params: IUmiPaginationParams) = runBlocking{
     ...
     * @return the inserted/updated record
     * */
    fun <T: Any> save(col: CoroutineCollection<T>, doc: T, cacheKey: String? = null, updateCache: Boolean = false) = runBlocking{
        col.save(doc)
        if (cacheKey != null){
     ...
     * */
    fun <T: Any> updateValues(col: CoroutineCollection<T>, filter: Bson, update: Bson, cacheKey: String? = null,
